...
{
	// if(!tsk) {tcp_send_reset(cb); return;}
	// 	log(DEBUG, "receive packet: now state = %s, packet control message %x, acknum = %d, send->next = %d", 
	//  tcp_state_str[tsk->state], cb->flags, cb->ack, tsk->snd_nxt);
	int state = tsk->state;
